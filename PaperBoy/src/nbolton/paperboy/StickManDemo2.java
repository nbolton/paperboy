...
public class StickManDemo2 extends BaseGameActivity implements IAccelerometerListener, IOnSceneTouchListener {
	...
	/*private void createLowerBody2() {
		...
		BodyPart torso = addBodyPart2(groinX, groinY, 180, 80);
		BodyPart leftLeg = addBodyPart2(groinX, groinY, 30, 100);
		BodyPart rightLeg = addBodyPart2(groinX, groinY, -30, 100);
	...
	private void createLowerBody1() {
		...
		BodyPart torso = addBodyPart1(centreX, centreY + 80, 0, 80);
	...
	private BodyPart addBodyPart1(float x, float y, float rotation, float length) {
...
/*
	private BodyPart addBodyPart2(float x, float y, float rotation, float length) {

		final Scene scene = mEngine.getScene();
		
		Rectangle shape = new Rectangle(x, y, 5, length);
		
		// rotate around one end, not the centre
		shape.setRotationCenter(x, y);
